...
function parseRequest(request) {
	var _request$split = request.split(CRLF + CRLF);
...
function unparseRequest(head, body) {
	return [head, body].join(CRLF.repeat(2));
}
...
function parseRequestLine(requestLine) {
	var method = requestLine.match(/^[A-Z]+\s/ig)[0];
	var httpVersion = requestLine.match(/\shttp\/\d+\.\d+$/ig)[0];
...
function parseUrl(rawUrl) {
	...
	if (query) {
		query.replace(/\s(.+)/g, '').replace(/\+/g, ' ').split('&').forEach(function (param) {
...
function parseCanonicalHeaders(rawHeaders) {
	...
	var canonicalHeadersString = signedHeadersList.map(function (key) {
		return [key, headersMap[key].sort().join(',')].join(':');
	}).join('\n');
